...
(function () {
  ...
  function waLink(message, interest) {
    ...
    return "/connect.html?interest=" + encodeURIComponent(interest || "nutrition") +
           "&note=" + encodeURIComponent(message);
  }

  function mailLink(subject, body) {
    if (!EMAIL_READY) return "/connect.html";
  ...
  (function () {
    ...
    host.innerHTML = live.map(function (p) {
      return '<a class="post reveal" href="/journal/' + esc(p.slug) + '.html">' +
